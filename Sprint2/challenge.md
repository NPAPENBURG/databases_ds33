...
The relationship between Employee and Terriorty would the Employess ReportsTo and the Terroiorty RegionID. Meaning that that employee reports to that section of region that is labled with an ID.


MongoDB would be useful in a situation where there is rapid growth in data that doesn't stay the same. Meaning that variables may change so this is a way to capture data so that you don't lose it. MongoDB is not useful when it comes to banks since banks like to have everything clearn cut and able to index into different tables.


NewSQL is a mix between SQL and NoSQL. NewSQL is a relational database that seeks to rpvoide the salabilty of NoSQL systems for online transaction procession while maintaining the ACID guarantees of a tradition database system. This is a new way to revoultionalize the problems with traditional online tranactions.
